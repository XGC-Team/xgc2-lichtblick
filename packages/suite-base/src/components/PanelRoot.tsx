...
type PanelRootStyleProps = Omit<PanelRootProps, "fullscreenState" | "selected"> & {
  embedded: boolean;
};

const useStyles = makeStyles<PanelRootStyleProps>()((theme, props) => {
  const { palette, transitions } = theme;
  const { sourceRect, hasFullscreenDescendant, embedded } = props;
...
export const PanelRoot = forwardRef<HTMLDivElement, PropsWithChildren<PanelRootProps>>(
  ...
  function PanelRoot(props, ref): React.JSX.Element {
    ...
    const { classes, cx } = useStyles({
      ...
      embedded: workspaceAppearance === "embedded",
    });
