...
export function validateTransformHistory(
  ...
  requireValue(
    bounds.maxStorageTime > 0n &&
      Number.isSafeInteger(bounds.maxCapacityPerFrame) &&
      bounds.maxCapacityPerFrame > 1,
    "Invalid native TF history bounds",
  );
  ...
  for (const row of history) {
    ...
    requireValue(
      record(message) && Array.isArray(message.transforms) && message.transforms.length === 1,
      "Offline TF messages must contain exactly one normalized transform",
    );
    const transform: unknown = message.transforms[0];
    requireValue(
      record(transform) &&
        record(transform.header) &&
        record(transform.header.stamp) &&
        typeof transform.child_frame_id === "string",
      "Invalid normalized TF",
    );
    ...
    requireValue(
      row.role === "tf-static" || (stamp > 0n && stamp === nanos(row.timeNs)),
      "Dynamic TF time mismatch",
    );
    ...
    requireValue(
      frame.times.size < bounds.maxCapacityPerFrame,
      `Native TF capacity would truncate ${id}`,
    );
    requireValue(
      frame.max - frame.min < bounds.maxStorageTime,
